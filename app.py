...
if uploaded_file:
    ...
    with st.spinner("🧠 Analyzing image..."):
        ...
        # "fake" < "real" alphabetically, so during training:
        #   class 0 = fake, class 1 = real
        # This means the model's sigmoid output = P(real), NOT P(fake).
        # We invert it here so `prob` consistently means P(fake)
        # for the rest of the app's logic.
        prob = 1 - prob
        # ──────────────────────────────────────────────────────
    ...
    with st.expander("🔧 Technical Details"):
        ...
        with col_a:
            st.markdown(f"""
            **Raw probability (P(fake)):** `{prob:.6f}`  
            **Threshold:** `0.5`  
            **Device:** `{device}`  
            **Model:** `EfficientNet-B0`  
            """)
