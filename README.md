...
![Python](https://img.shields.io/badge/Python-3.9+-3776AB?style=for-the-badge&logo=python&logoColor=white)# DeepFake Detector 🔍
...
🚀 **[Live Demo](https://swetha-deepfake-detector.streamlit.app/)**
...
## Label Inversion Bug Discovery

After deployment, real-world testing revealed every uploaded photo — including my own selfies and even genuine "real" samples pulled directly from the training dataset — was being classified as **FAKE with 100% confidence**.

**Debugging process:**
1. Ruled out camera beautification filters (tested with filters disabled)
2. Ruled out image compression artifacts (tested with an uncompressed webcam capture)
3. Tested a known **real** image from the original training set → still predicted FAKE
4. Tested a known **fake** image from the training set → predicted REAL

Step 4 confirmed a **consistent, complete inversion** — not a data quality issue, but a labeling bug in the inference logic.

**Root cause:** PyTorch's `ImageFolder` assigns class indices alphabetically. Since `"fake"` precedes `"real"` alphabetically, the model was trained with `fake=0, real=1` — meaning the sigmoid output represented `P(real)`, not `P(fake)` as the inference code assumed.

**Fix applied:** Inverted the probability interpretation (`prob = 1 - prob`) immediately after the model's forward pass, correcting every downstream calculation without needing to retrain the model.

**Lesson learned:** A model can be statistically excellent (99.98% validation accuracy) while still being completely wrong in production due to a labeling/interpretation mismatch — validation metrics alone don't guarantee correct real-world behavior. Always sanity-check predictions against known ground-truth samples after deployment.
...
- Model exhibits overconfidence (near-100%/0% predictions), typical of deep binary classifiers — a known calibration limitation addressable via temperature scaling
...
[GitHub](https://github.com/ixsntg012-lab) · [Live Demo](https://swetha-deepfake-detector.streamlit.app/)
